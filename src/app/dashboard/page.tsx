...
interface RawSupabaseSubmission {
    id: number;
    created_at: string;
    feedback_status: string; // <-- FIX 1: Interface updated
...
export default async function DashboardPage() {
    ...
    const { data: rawInitialSubmissions, error: fetchError, count: rawInitialTotalCount } = await supabase
        ...
        .select(`
            ...
            overall_score,
            feedback_status 
        `, { count: 'exact' }) // <-- FIX 2: Query updated
        .eq('user_id', user.id)
    ...
    const initialSubmissionsForList: SubmissionItemData[] = (rawInitialSubmissions || []).map((sub: RawSupabaseSubmission) => {
        ...
        return {
            ...
            feedback_status: sub.feedback_status // <-- FIX 3: Property added to returned object
        };
    });
