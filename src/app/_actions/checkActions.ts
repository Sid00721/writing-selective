...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!, {
  apiVersion: "2025-04-30.basil", // Use the version TS expects, or update library
  typescript: true,
});
...
export async function createCheckoutSession(): Promise<ActionResult> {
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.error(
      "Checkout Action Error: Supabase URL or Anon Key environment variables are missing!"
    );
  ...
  console.log(
    "[Server Action Runtime] Attempting to create Supabase client by passing vars..."
  );
  ...
  if (authError || !user) {
    console.error(
      "Checkout Action Error: No user session found or auth error:",
      authError
    );
  ...
  if (profileFetchError && profileFetchError.code !== "PGRST116") {
    ...
    console.error(
      "Checkout Action Error: Error fetching profile:",
      profileFetchError
    );
  ...
  if (!customerId) {
    try {
      console.log(
        `[Server Action Runtime] Creating Stripe customer for user ${user.id}`
      );
      ...
      console.log(
        `[Server Action Runtime] Stripe customer ${customerId} created.`
      );

      // Save the new customer ID back to the user's profile
      console.log(
        `[Server Action Runtime] Saving Stripe customer ID ${customerId} to profile for user ${user.id}`
      );
      ...
      if (updateError) {
        console.error(
          "Checkout Action Error: Failed to save Stripe Customer ID to profile:",
          updateError
        );
        // Set profileError, but continue to try creating checkout session if possible
        profileError =
          "Could not save customer details. Please contact support if payment succeeds.";
      } else {
        console.log(
          `[Server Action Runtime] Saved Stripe customer ID ${customerId} for user ${user.id}`
        );
      }
    } catch (e: unknown) {
      console.error(
        "Checkout Action Error: Error creating Stripe customer:",
        e
      );
      ...
      return {
        error: "Could not create customer record due to an unexpected error.",
      };
    }
  } else {
    console.log(
      `[Server Action Runtime] Found existing Stripe customer ID ${customerId} for user ${user.id}`
    );
  }
  ...
  if (!priceId) {
    console.error(
      "Checkout Action Error: NEXT_PUBLIC_STRIPE_PRICE_ID environment variable is missing!"
    );
  ...
  try {
    console.log(
      `[Server Action Runtime] Creating Checkout session for customer ${customerId} with price ${priceId}`
    );
    ...
    console.log(
      `[Server Action Runtime] Checkout session ${session.id} created.`
    );

    if (!session.url) {
      console.error(
        "Checkout Action Error: Stripe session created but URL is missing."
      );
    ...
    console.error(
      "Checkout Action Error: Error creating Stripe Checkout session:",
      e
    );
    if (e instanceof Error) {
