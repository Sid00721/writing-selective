...
const WritingSession: React.FC<WritingSessionProps> = ({ currentPrompt }) => {
  ...
  const submitWriting = useCallback(async () => {
    ...
    if (authUserError) { // Log 5: Check auth user error
      console.error("SUBMIT WRITING: Error getting user:", authUserError.message);
      toast.error('Authentication error. Please try again.', { id: loadingToastId, icon: <AlertCircle /> });
    ...
    console.log("SUBMIT WRITING: Current editor content (JSON string):", currentContentJson); // Log 8
    ...
    try {
      ...
      if (insertError || !insertResult) {
        console.error("SUBMIT WRITING: Supabase insert error:", insertError?.message || 'No insert result returned.'); // Log 16
        toast.error(`Submission failed: ${insertError?.message || 'Unknown error during insert.'}`, { id: loadingToastId, icon: <AlertCircle /> });
        setIsSubmitting(false); 
      } else {
        const newSubmissionId = insertResult.id;
        console.log("SUBMIT WRITING: Submission successful. New ID:", newSubmissionId); // Log 17
        ...
        generateFeedbackForSubmission(newSubmissionId)
          .then(res => console.log("SUBMIT WRITING: Feedback generation action result:", res))
          .catch(err => console.error("SUBMIT WRITING: Error calling feedback action:", err));
        ...
        console.error("SUBMIT WRITING: Unexpected error during submission process:", unexpectedError);
        toast.error('An unexpected error occurred. Please try again.', { id: loadingToastId, icon: <AlertCircle /> });
        setIsSubmitting(false);
    }
    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 py-3 border-b border-gray-200">
        ...
        <div className="flex-shrink-0 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto mt-3 md:mt-0">
          ...
          <Timer initialMinutes={30} onTimeUp={handleTimeUp} showTextLabel={false} />
        </div>
      </div>
